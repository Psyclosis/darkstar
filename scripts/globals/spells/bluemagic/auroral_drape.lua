...
function onSpellCast(caster,target,spell)
    ...
    if (target:hasStatusEffect(dsp.effects.BLINDNESS) and target:hasStatusEffect(dsp.effects.SILENCE)) then
        ...
        if (BLIND_RES > 0.125) then
            target:addStatusEffect(dsp.effects.BLINDNESS,60,0,60);
            effect = EFFECT_BLINDNESS;
            MSG = msgBasic.MAGIC_ENFEEB; -- Landed it.
        end

        if (SILENCE_RES > 0.125) then
            target:addStatusEffect(dsp.effects.SILENCE,20,0,60);
