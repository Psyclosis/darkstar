...
function onTrigger(player, option)
    ...
    elseif (option == "restore") then
        if (player:hasStatusEffect(dsp.effects.ATMA) or player:getVar("ACTIVE_ATMA_COUNT") > 0) then
    ...
    elseif (option == "save") then
        ...
        if (player:getVar("ACTIVE_ATMA_COUNT") > 0) then
            if (player:hasStatusEffect(dsp.effects.ATMA)) then
                player:setVar("PREV_ATMA_1", player:getStatusEffect(dsp.effects.ATMA):getPower())
            end
            if (player:hasStatusEffect(dsp.effects.ATMA_2)) then
                player:setVar("PREV_ATMA_2", player:getStatusEffect(dsp.effects.ATMA_2):getPower())
            end
            if (player:hasStatusEffect(dsp.effects.ATMA_3)) then
                player:setVar("PREV_ATMA_3", player:getStatusEffect(dsp.effects.ATMA_3):getPower())
            end
